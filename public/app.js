...
const I = {
  ...
  pin:      (s) => _svg('<path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0Z"/><circle cx="12" cy="10" r="3"/>', s),
  briefcase:(s) => _svg('<rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/>', s),
    ...
    <div class="bsheet-section-body">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
        ...
        <div>
          ...
          <p class="stop-meta" style="margin:2px 0 0;">Versione 5.125 &mdash; settembre 2026</p>
        </div>
      </div>
      ...
      <ul class="info-list">
        <li><b>Niente più doppioni</b>: modificare un giro salvato (data, tappe) o un cliente in anagrafica non crea più una copia; anche i giri multi-giorno si aggiornano invece di duplicarsi.</li>
        <li><b>Pausa pranzo</b>: cambiare il locale non sposta più l'orario; se togli l'orario fisso "alle", il pranzo resta (spezza una tappa lunga a metà giornata invece di sparire); eliminarlo lo tiene eliminato.</li>
        <li><b>Soste automatiche</b>: ora si possono togliere dal giro ("Togli soste") e non ritornano al ricalcolo.</li>
        <li><b>Riordino tappe</b>: una tappa spezzata dal pranzo non perde più metà durata; si conservano finestra oraria e "prima tappa".</li>
        <li><b>Impostazioni</b>: salvarle non sovrascrive più la partenza/arrivo impostati a mano.</li>
        <li>Recupero dati: importati i giri/contatti rimasti sul dispositivo e dal database precedente.</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Multi-giorno: ripristinata la costruzione per ZONA (valle) — che teneva insieme le tappe della stessa valle senza mescolare valli lontane — e aggiunta l'unione parziale sul corridoio: una giornata lontana poco piena prende singole tappe "sulla via" da una valle adiacente, senza svuotarla. Le giornate sono di nuovo coerenti per valle e più piene.</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Fix definitivo barra navigazione inferiore su iOS: body bloccato al viewport, solo .app scrolla internamente — la tab bar non si muove mai durante lo scroll, neanche con future modifiche al layout.</li>
      </ul>

      <p style="font-weight:600;font-size:0.85rem;margin-top:14px;margin-bottom:6px;">Novità v5.051</p>
      <ul class="info-list">
        <li>Schede Sosta e Pausa pranzo: clic sulla card apre direttamente Maps con la ricerca corretta (bar o ristorante vicino). Rimossi i due pulsanti separati; piccola icona segnaposto indica l'azione.</li>
      </ul>

      <p style="font-weight:600;font-size:0.85rem;margin-top:14px;margin-bottom:6px;">Novità v5.050</p>
      <ul class="info-list">
        <li>Fix avvio server: rimossa dichiarazione const duplicata in index.js che causava SyntaxError al boot — il server non si avviava e il login restituiva errore su Safari.</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Fix duplicazione giro: ricalcolo ordine tappe e toggle pausa pranzo ora aggiornano il giro esistente invece di crearne uno nuovo; state.route.id tiene traccia del giro in modifica nel tab Percorso.</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Multi-giorno: ripristinata la costruzione per ZONA (valle), che teneva insieme le tappe della stessa valle e univa solo le giornate adiacenti compatibili. Il tentativo successivo (costruzione "a tappe") sul giro reale attraversava valli opposte e separava le tappe gemelle, quindi è stato annullato. Le giornate ora sono di nuovo coerenti per valle.</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Tappa lavorata durante la chiusura: quando l'intervento non sta interamente nel pomeriggio, la tappa viene ora spezzata sulla pausa di chiusura (es. 09:00–13:00 / 14:30–16:00, arrivo 12:16 → lavoro 12:16–13:00 e 14:30–15:46) invece di proseguire ignorando l'orario. Il gap di chiusura diventa così la pausa pranzo naturale, che prima non veniva inserita</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Avviso quando un intervento sfora la chiusura: se le soste accumulate spostano l'inizio del lavoro oltre l'apertura e il servizio finisce dopo l'orario di chiusura, la tappa mostra un avviso invece di sforare in silenzio</li>
      </ul>

      <p style="font-weight:600;font-size:0.85rem;margin-top:14px;margin-bottom:6px;">Novità v4.118</p>
      <ul class="info-list">
        <li>Arrivo dopo chiusura mattutina: se non c'è abbastanza tempo prima della chiusura (< 45 min), l'intervento viene spostato interamente al pomeriggio invece di creare uno spicchio di lavoro troppo breve che i ritardi da soste renderebbero impossibile</li>
        <li>Pranzo durante attesa apertura: quando si arriva vicino a una tappa che non è ancora aperta e c'è tempo prima dell'apertura, il planner cerca automaticamente un ristorante in zona per non sprecare quell'attesa</li>
      ...
      <ul class="info-list">
        <li>Fix orari tappa: se si arriva durante la pausa pranzo (chiusura 13:00–14:00) l'intervento viene spostato all'apertura pomeridiana invece di sovrapporsi alla chiusura</li>
        <li>Fix orari tappa: se l'intervento supera la chiusura mattutina ma lo split non è possibile, l'intervento viene spostato al pomeriggio</li>
      </ul>
      ...
      <ul class="info-list">
        <li>Fix ordine tappe: ricalcolo da vista risultato ora preserva l'ordine del giro caricato (manualOrder=true)</li>
        <li>Meteo: timeout fetch aumentato a 6s (era 2.5s) — MeteoTrentino e Open Data Hub non vanno più in timeout</li>
        <li>Open-Meteo: deduplicazione in-flight — più tappe in parallelo con stesse coordinate usano un'unica richiesta HTTP; elimina 429</li>
      </ul>
  ...
  return state.addresses.map(a => {
    ...
    return `
      <article class="card archive-card${isSel ? " archive-card--selected" : ""}" ${sel ? `data-select-address="${a.id}" style="cursor:pointer;"` : ""}>
        ...
        ${a.activity ? "" : `<div class="stop-meta">${I.briefcase(13)} <span class="archive-missing">Attività non indicata</span></div>`}
        <div class="stop-meta"${a.location ? ` style="font-weight:600"` : ""}>${I.pin(13)} ${a.location
          ? escapeHtml(a.location)
          : `<span class="archive-missing">Sede non indicata</span>`}</div>
