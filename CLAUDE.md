...
- **Conversione** (`rebuildStopsFromResultRows`):
  - **SOSTA** scelta **manualmente da Maps** (`userPicked:true`) → diventa una **tappa fissa persistente** (il planner non può ri-derivarla, non è in archivio).
  - **PRANZO scelto a mano (v5.085)**: NON diventa una tappa. Cambiare il locale del pranzo deve tenere l'**orario** invariato (bug storico: diventava una tappa senza finestra e, se un cliente era spezzato dalla chiusura, il merge lo spostava a fine giornata). Ora il locale scelto viaggia come **`lunchFixedSpot`** nel payload `/api/plan` e il replan usa **`lunchBreak:true`**: il planner (`makeLunchEntry`, short-circuit in cima) tiene la posizione **temporale** calcolata (metà giornata / gap chiusura) e forza **solo** la posizione geografica. Persistenza: la riga pranzo materializzata porta `userPicked:true`; `replanFromResult`/`replanWithOrder` ri-derivano `lunchFixedSpot` da quella riga (niente stato residuo tra giri diversi grazie ai reset). Eliminare il pranzo → `lunchBreak` segue lo stato corrente del giro (eliminato resta eliminato).
...
- Le righe break materializzate (`planner.js`, blocco `result.push`) propagano `weeklyHours`, `notes`, `addressId`, `placeAssigned`, `userPicked`.
- **Soste automatiche on/off per giro (v5.087)**: le soste (`type:"rest"`) sono ri-derivate a ogni replan dalla guida cumulata, quindi eliminarne una la faceva **ritornare**. Ora esiste il flag di giro **`restBreaks`** (default `true`): il payload `/api/plan` lo passa, `insertBreaks` salta ogni sosta automatica se `false` (guardia in cima a `tryInsert`), e `planRoute` lo **echo-a** nel risultato (persiste al salvataggio). Client: pulsante **`#toggle-rest-breaks`** ("Togli soste"/"Soste auto") nella vista risultato + `state.resultRestEnabled` (tri-stato, come `resultLunchEnabled`); eliminare una sosta **automatica** (`!userPicked`) disattiva `restBreaks` per il giro (le soste **scelte a mano** restano tappe reali e si eliminano singolarmente).
