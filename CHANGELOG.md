## v5.071 — 2026-06-30
Coerenza "login obbligatorio + local-first dietro la sessione" (niente più utenti anonimi):
- **Accesso obbligatorio**: rimosso l'utente fittizio `{username:"locale", id:0}` e il flag `_syncEnabled`. Login/registrazione sono ora la schermata d'ingresso; senza sessione l'app non entra in modalità operativa. Il sync è sempre attivo quando online (gate su `state.user`, non più su un flag).
- **Fallback chiuso lato server**: rimosso `userId = (await authenticate()) || 1` in `server/index.js`. Tutti gli endpoint dati rispondono **401** alle chiamate non autenticate — niente più bucket condiviso `user_id=1`. `/api/config` resta pubblico (solo chiavi browser pubbliche); `/api/health` invariato.
- **Avvio offline-first**: marcatore di sessione non sensibile in `localStorage` (`session.user`). Se presente, l'app parte SUBITO con i dati locali (IndexedDB) senza check di rete bloccante e ri-valida la sessione in background (`revalidateSession`) su `online`/`pageshow`/`visibilitychange`. Una 401 dal server riporta al login senza cancellare i dati locali.
- **Migrazione dati locali**: al primo accesso dopo l'update, i dati creati quando l'app era anonima (solo in IndexedDB, outbox mai popolato) vengono **adottati** nell'account reale accodandoli all'outbox e svuotandolo, con riconciliazione degli id dal server (`adoptLocalDataIntoAccount`). Non distruttivo, una volta sola per dispositivo. La push (adozione) avviene PRIMA del primo pull per evitare che un pull da account vuoto sovrascriva i dati locali.
- **Condivisione + import autenticati**: `GET /api/share/:token` ora richiede sessione (401+`needsAuth`); il destinatario senza login viene mandato al login e, dopo l'accesso, l'import riprende automaticamente (`pendingShareImport`). Il giro importato è uno **snapshot indipendente**: salvato tra i propri giri come giro nativo (sync incluso), con tappe **self-contained** (`addressId` azzerato → niente fusione/collisione con l'anagrafica del destinatario). Il calcolo percorso usa le coordinate embedded.
- **Scadenza link → 7 giorni** (`SHARE_LINK_TTL_DAYS`, era 5). La scadenza blocca solo nuovi import; le copie già importate restano permanenti.
- `POST /api/routes` ora persiste `source` (es. `"imported"`) anche lato server.
- Fuori scopo (invariato): nessun accoppiamento a MEDIS.
