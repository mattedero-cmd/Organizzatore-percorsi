...
- **Nuovo `scripts/replay-multiday-eni.mjs`**: ricostruisce i tempi REALI dal log ENI 202609 (tempi da casa + vicini stampati + ancoraggi imposti dai fatti del log, Floyd-Warshall), sostituisce Google con uno stub su quella matrice e fa girare il **motore vero**. Confronta anche una variante: `node scripts/replay-multiday-eni.mjs /percorso/alt.js`.
- **Fedeltà verificata** (5 controlli su 5, `DUR=10`): 4 zone identiche al log, composizione **1/13/3**, Malé isolata 07:00–10:32 (log 10:38) e **partenza della giornata da 13 tappe alle 05:22 identica al log** — firma forte: casa→Silandro e il calcolo a ritroso combaciano. Km 886 contro 939 (−5,6%), scarto dovuto alla conversione tempo→km dello stub, che non tocca le decisioni di clustering.
- **Caso "giornata Estremi" — indagato, nessuna modifica al motore**: sul giro reale l'app produce 1/13/3 (una giornata da 1 tappa con 472' di margine, una da 13) contro il 7/4/4 costruito a mano dall'utente. Il piano dell'utente è **fattibile e migliore** (992–1002' di guida contro 1052'), confermato da quattro verifiche indipendenti. Ma i quattro rimedi sviluppati sono stati **falsificati con controesempi eseguiti** (ricerca locale sui km = `improveClusters`: 7 nuovi zigzag e 13 giornate da 1 tappa su 100 geometrie; pavimento sulla partenza: sull'altro giro reale 5→6 giornate e +138 km; fase "estremi": guadagno consegnato zero) e il prototipo, provato col nuovo replay, è risultato un **no-op** (stesso identico 1/13/3). Diagnosi, numeri e vicoli ciechi sono ora in `docs/MULTI_GIORNO.md` perché non si ripercorrano alla cieca.
...
- **Reperto 1 — l'affermazione di sicurezza era falsa.** Il commento diceva «sovra-accorpare vicino casa è sicuro per costruzione, growDays rispezza da solo con l'oracolo reale». Non è vero: `growDays` spezza per **fattibilità oraria**, mai per direzione o per economia (il criterio di corridoio di v5.014 fu revertito), e nessuna fase a valle sa smontare una giornata vicino-casa già compatta. Su geometrie "a stella" — valli che si toccano solo passando da casa, cioè esattamente il modello dell'utente — v5.118 regrediva in **21 casi su 180**, fino a +81 km e a una giornata in più.
- **Reperto 2 — la guardia era vacua.** `NEAR_HOME_DIAMETER = 70` non è mai entrata in azione: per disuguaglianza triangolare un gruppo entra nella zona di un seme solo se `d(g,seme) ≤ d(g,casa)`, quindi `seedHome ≤ 2 × nearHomeT`; con `nearHomeT ≤ 35` la seconda clausola è sempre soddisfatta. Verificato su **4000 matrici metriche casuali: 0 attivazioni**, rapporto massimo osservato 1,989 contro il limite teorico 2. Rimossa invece di lasciare una falsa protezione.
- **Reperto 3 — il gate di `fillPartial` non era il motore del risultato**, e in un giro reale plausibile (lo stesso senza tre clienti) **costava una giornata intera**.
- **Il fix — niente più scommesse**: si costruiscono **quattro piani** (accorpamento vicino-casa "storico" o "membro più vicino" × gate di economia acceso o spento) e si tiene quello con **meno giornate**; a parità, meno guida; a parità di tutto vince il comportamento storico. Poiché una delle quattro varianti *è* l'algoritmo di prima, il risultato **non può mai essere peggiore di prima** per numero di giornate. Costo: zero chiamate Google in più (la matrice è già in cache), 0,14 s in più sul giro reale.
- **Misurato su 180 geometrie casuali a stella**: −21 giornate (437 → 416) a +150 km totali su 60.750 (+0,25%), e **nessun caso con più giornate di prima** (erano 21). La Diagnostica ora stampa una riga per variante con giornate e guida, così la scelta è verificabile.
- **Spareggio di coerenza di valle** in `dissolveDays`: la giornata ricevente era un puro argmin di Δguida, quindi a parità sostanziale di costo poteva vincere un'altra valle. Ora, entro 20 minuti di Δ, vince quella con il **detour dal corridoio** minore — cioè quella sulla cui via di casa il gruppo si trova davvero. È uno spareggio, non un veto: non può far fallire una dissoluzione.
- **Tappe a sede chiusa finalmente visibili**: il piano su più giorni calcolava `lateStops` per ogni giornata ma **non le mostrava da nessuna parte** — finivano solo nel log della Diagnostica. Ora ogni giornata che serve una tappa fuori orario lo dice in chiaro. Serve perché il giorno della settimana di una giornata dipende da quante giornate la precedono: eliminarne una sposta tutte le successive, e una composizione identica può passare da martedì a lunedì e trovare chiuso (misurato: 4 scenari su 20 con orari settimanali reali).
- Rimosso un blocco di commento duplicato lasciato da v5.118, e documentato che il gate di `fillPartial` lascia passare la mossa quando una delle due misure manca.

## v5.119 — 2026-09-06
Le modifiche in anagrafica si vedono subito nei giri già salvati, senza rifarli.
- **Bug**: un giro salvato conserva una **fotografia** delle tappe presa al momento della pianificazione. Correggendo poi il cliente in anagrafica — indirizzo, telefono, orari di apertura, coordinate — il giro continuava a mostrare i dati vecchi, e l'unico modo per vederli aggiornati era **ricreare il giro**.
- **Fix**: l'**archivio è la fonte di verità** per i dati del contatto. Ogni riga di un giro con `addressId` viene ri-allineata al contatto nel momento in cui il giro si mostra (`hydrateRowFromArchive`, agganciata a `normalizeSavedRoute`, che è il passaggio obbligato di tutte le viste: risultato, stampa/PDF, WhatsApp, elenco giri). Niente da salvare e niente da ricreare: si apre il giro e i dati sono quelli giusti.
- **Cosa si aggiorna e cosa no**: si aggiornano solo i campi del **contatto** (cliente, attività, sede, indirizzo, coordinate, telefono, email, orari settimanali e i relativi orari piatti). Restano intatti quelli del **giro**: durata dell'intervento, finestra oraria, "ignora orari", "prima tappa", **note della tappa** (che dalla v5.097 sono per-giro, non del contatto) e tutti i valori calcolati (orari, km, tempi di guida).
- **Cosa non viene toccato**: le pause pranzo e le soste, le tappe provvisorie (senza `addressId`) e i **giri importati**, che per progetto hanno gli `addressId` azzerati e sono self-contained — non hanno un contatto in archivio a cui allinearsi. Se il contatto è stato **eliminato**, la fotografia resta com'era invece di svuotarsi.
- **Avviso quando serve ricalcolare**: se è cambiato il **punto** (indirizzo o coordinate), orari e chilometri calcolati non valgono più. Invece di mostrare numeri sbagliati in silenzio, la vista risultato dice *"L'indirizzo di X è cambiato in anagrafica: orari e chilometri qui sotto sono ancora quelli vecchi"* con un pulsante **Ricalcola il giro**. Il ricalcolo resta una scelta dell'utente, perché consuma chiamate API. Se cambia solo il telefono o il nome, nessun avviso: non c'è nulla da ricalcolare.
- **Anche il multi-giorno**: `recalcSavedMultiDay` riallinea le tappe salvate all'archivio prima di ripianificare, altrimenti il ricalcolo di un giro salvato usava indirizzi, orari e coordinate vecchi.
- Verificato con 24 casi eseguiti sul sorgente reale: aggiornamento di cliente/indirizzo/telefono/email/coordinate/orari; durata, finestra oraria, flag e note della tappa preservati; valori calcolati intatti; pause, tappe provvisorie e giri importati non toccati; contatto eliminato → fotografia conservata; solo-telefono-cambiato → nessun avviso; forma "tappa" (`fullAddress`) del multi-giorno gestita senza inventare chiavi.

## v5.118 — 2026-09-06
Multi-giorno: da 5 giornate e 997 km a 3 giornate e 940 km sul giro reale della Diagnostica.
- **Il problema (Diagnostica reale 2026-09-06, 17 tappe)**: 5 giornate, di cui **tre quasi vuote** — chiuse alle 08:59, 07:18 e 10:38 con 571/672/472 minuti di margine su una finestra 07:00–18:30. L'utente: «può essere diviso in 3 giornate e 100 km in meno».
- **Metodo — prima l'harness, poi il codice**: il vincolo storico «offline non c'è la chiave Google, quindi i test non riproducono la produzione» è stato superato con un **replay** che intercetta `globalThis.fetch` sulle Directions e ricostruisce la matrice (grafo stradale + Floyd-Warshall, ancorata ai valori esatti del log). Col codice **pre-fix** riproduce la Diagnostica reale: **20/20 fatti, 43/44 righe byte-identiche, km per giornata a scarto 0,0%, fine giornata a scarto 0 minuti**. Ogni correzione è stata misurata su quello. Vedi `docs/MULTI_GIORNO.md`.
- **Causa 1 (decisiva) — `assignZones` guardava il membro sbagliato**: il merge "vicino casa" testava `seedHome`, cioè il tempo-casa del **seme**, che per costruzione è il gruppo **più LONTANO** della zona. Così una zona era "vicino casa" solo se il suo membro più lontano stava entro 35': San Michele a/A a **37'** — due minuti oltre — trascinava fuori dal vicinato Trento 26' e Trento 27', e Ala a 54' trascinava fuori Rovereto 33' e 40'. Nove tappe tutte entro un'ora da casa finivano in TRE zone e quindi in TRE giornate, perché `growDays` lavora una zona alla volta e **il numero di zone è un pavimento sul numero di giornate**. **Fix**: si testa il membro più VICINO, con una guardia di diametro (70') perché un corridoio lungo che sfiora casa non venga inghiottito. Sovra-accorpare vicino casa è sicuro (growDays rispezza da solo con l'oracolo reale); sotto-accorpare no, perché nessuna fase a valle sa ricomporre.
- **Causa 2 — `fillPartial` non guardava il prezzo**: il gate anti-furto verificava solo che il donatore restasse non-vuoto e fattibile. La giornata di Canazei si è presa ENIMOOV+Bolzano ed Eni Station da quella di Silandro pagandole 37' e 50' di guida, mentre il donatore ne risparmiava **5' e 9'** (Bolzano è sulla via di casa da Eni Station: 1'). Silandro restava **sola con 242' di guida** e finiva accoppiata a Malé in una giornata da 357 km: la fase nata per riempire le giornate povere ne creava una più povera. **Fix**: gate di **economia globale** — la mossa è respinta se costa al ricevente più di quanto risparmia al donatore (tolleranza 10'), col conto stampato in Diagnostica.
- **Causa 3 — `dissolveDays`, tetto piatto contro criterio economico**: `DISSOLVE_GROUP_DETOUR` = 60' assoluti scavalcava `gain = dDrive − totalDelta ≥ 30'`, che invece scala. La giornata di Malé costa 203' di guida ed è **un solo gruppo**: l'economia avrebbe concesso 173' di Δ, il tetto ne concedeva 60 → serviva un guadagno di 143' invece di 30, **4,8× più severo**. Da qui il «senza giornata ricevente». **Fix**: budget **residuo** `max(60, dDrive − MIN_GAIN − speso)`; il criterio economico aggregato resta l'unico giudice, quindi non può accettare nulla che l'economia non accetti già. Esito: Malé assorbita nella giornata di Silandro, **~66' di guida risparmiati**.
- **Risultato misurato**: G1[9 tappe vicino casa] 187 km · G2[Silandro, Eni Station, ENIMOOV, Bolzano, Malé] 386 km · G3[Canazei, Cavalese, Vipiteno] 366 km = **3 giornate, 940 km** (da 5 e 997).
- **Provato e RIMOSSO**: scegliere l'estremo di partenza fra i k più lontani in `orderDayFarFirst`. Sembrava valere ~68 km sulla giornata Fassa/Isarco, ma sull'harness Canazei→Cavalese→Vipiteno e il suo inverso costano **entrambi 393'**: la stima si reggeva su una congettura geografica non supportata dai dati. Non si tiene un cambiamento che non guadagna nulla sull'unico giro validabile.
- ⚠️ **CORRETTO in v5.120 dopo verifica avversaria**: due affermazioni di questa voce erano sbagliate. (a) Il salto 5→3 giornate e i −57 km sono **interamente della Causa 1**; il gate di `fillPartial` (Causa 2), da solo sopra la Causa 1, era anzi una **regressione di una giornata**, mascherata dalla Causa 3 spedita nello stesso commit. (b) La frase «Silandro è rimasta SOLA con 242' di guida ed è finita accoppiata a Malé in una giornata da 357 km» descrive uno stato che nella Diagnostica reale **non esiste**: lì Silandro finisce con San Michele/Trento/Trento/Ravina in 264 km. Quella giornata compare solo DOPO la Causa 1 ed è una delle tre del piano che questa versione rivendica.
- **Documentato ma NON corretto** (in `docs/MULTI_GIORNO.md`): (a) la directness è **strutturalmente cieca vicino casa** — per un gruppo perfettamente sul corridoio il minimo possibile è `(S−g)/(S+g)`, quindi `dir ≤ TAU` equivale a `g ≥ S·(1−TAU)/(1+TAU)`: con TAU 0.45 e seme a 127', **nessuna tappa a meno di 48' da casa può passare, nemmeno a detour zero** (erano 8 tappe su 17, escluse in silenzio); (b) il calcolo a ritroso di `planner.js` non ha pavimento a `startTime`, per questo il Giorno 2 partiva alle **04:26** con orario impostato 07:00 — tocca il motore della giornata singola, quindi va deciso insieme.

## v5.117 — 2026-09-06
Se una tappa ha le coordinate GPS, il navigatore ci va SEMPRE sopra (non più sull'indirizzo scritto).
- **Bug**: "Naviga" e "Apri percorso completo" costruivano gli URL del navigatore **solo dal testo** (`row.address`, oppure `cliente + sede`), buttando via le coordinate salvate sulla tappa. Con insegne diffuse ("Eni", decine di sedi), civici che Maps non trova o frazioni omonime, il navigatore poteva portare **a chilometri di distanza** dal punto scelto sulla mappa. (Le schede di sosta/pranzo usavano già il GPS: era un'incoerenza.)
- **Fix**: nuovo `navPoint()` — se la tappa ha lat/lng valide si passa `lat,lng` al navigatore, altrimenti si ripiega sul testo. Usato da `routePoints()` (percorso completo: partenza, tutte le tappe, arrivo), da `stopNavUrl()` (pulsante Naviga della singola tappa) e dalle schede sosta/pranzo, ora tutte sulla stessa strada.
- **Waze**: con le coordinate si usa `ll=` (il parametro dedicato al GPS) invece di `q=`, che è una ricerca testuale. Inoltre il pulsante **Naviga** della singola tappa rispetta finalmente Waze: prima, con Waze impostato come navigatore, apriva comunque Google Maps (il ramo mancava).
- **Ricalcolo**: `rebuildStopsFromResultRows` recupera lat/lng dall'archivio quando la riga non le ha (come già faceva per telefono e orari) — una tappa salvata prima che il contatto fosse geocodificato torna così ad avere il GPS.
- Già corretti e lasciati invariati: il motore di pianificazione (`resolvePlace` restituisce subito le coordinate salvate, quindi Google Directions riceve già `lat,lng`) e la mappa del risultato (usa le coordinate della riga, poi l'archivio, e geocodifica solo come ultima risorsa).
- Verificato con 14 casi eseguiti sul sorgente reale: indirizzo+GPS → GPS; solo indirizzo → testo; `null`/`0,0`/stringhe numeriche gestiti; percorso completo con GPS dove c'è e testo dove manca, pranzo escluso; URL Google/Apple/Waze per percorso intero e singola tappa.
